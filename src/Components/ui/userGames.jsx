      ...
      <div
        ...
        <div className="">
          <header className="flex justify-between items-center pr-4 pb-2 font-medium">
            <p className="text-lg">{lobby_name}</p>
            <p className="text-lg">{game_name}</p>
            <p className="text-xs">{game_mode}</p>
          </header>
        </div>
        { game.owner_id == Cookies.get('ssid') &&
        <div className="flex gap-2">
          <button className="btn btn-active" onClick={() => {}}>Del</button>{' '}
          <button className="btn btn-active" onClick={() => {}}>Edit</button>{' '}
          <button className="btn btn-active" onClick={() => {}}>Start</button>{' '}
        </div>
        }
      </div>
    );
